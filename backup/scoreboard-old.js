var Scoreboard = Vue.component('scoreboard', {
  template: `
    <div id="canvas">
    <div v-if="scoreboard_data.length == 0" class="row">
    ...
    <div v-else>
        <div class="p-2 m-0" id="masthead">
            <div class="row no-gutters justify-content-md-center align-items-center">
                <div class="col-md-auto">
                    <h3 class="text-white p-1 mt-0  text-center">{{ event_title }} Round {{currentRound}}</h3>
                </div>
                <div class="col col-lg-1">
                    <b-pagination class="float-right mt-3" align="center" :total-rows="+total_players" @input="processDetails"
                        v-model="currentPage" :per-page=per_page :hide-ellipsis="true" size="sm" :limit=1 aria-label="Next 15"
                        v-b-tooltip.hover title="Navigate to next/previous page" />
                </div>
                <div class="col col-lg-2">
                    <button type="button" class="btn btn-dark p-2 float-left" v-on:click="reload()"><i class="fa fa-refresh" v-bind:class="{'fa-spin fa-fw': reloading == true}"
